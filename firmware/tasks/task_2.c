...
#define TARGET_FREQUENCY_6_25_Hz_CODE TARGET_FREQUENCY_MAX + 1
#define TARGET_FREQUENCY_6_25_Hz 625
#define TARGET_FREQUENCY_10_4_Hz_CODE TARGET_FREQUENCY_MAX + 2
#define TARGET_FREQUENCY_10_4_Hz 104
...
int16_t EEMEM eep_target_frequency_task_2;
int16_t EEMEM eep_target_amplitude_task_2;
int16_t EEMEM eep_target_mode_task_2;
...
// Синтез треугольных сигналов для 9-ти каналов ЭКС
void task_2(void)
{
    task_read_settings();
    ...
    else if (target_frequency == TARGET_FREQUENCY_6_25_Hz_CODE)
    {
        phase_inc = pgm_read_dword(&phase_inc_factors[19]);
    }
    else if (target_frequency == TARGET_FREQUENCY_10_4_Hz_CODE)
    {
        phase_inc = pgm_read_dword(&phase_inc_factors[20]);
    }
    else
    {
    ...
    while (task_control(&state_menu, &state_signal, true))
    {
        ...
        if (state_signal == STATE_SIGNAL_GENERATE && !is_set_new_dac_values)
        {
            ...
            high_phase = (uint16_t)(phase_acc >> 16);

            if (high_phase < 32768)
            {
                value = (high_phase >> 3); // Вверх: 0..32767 -> 0..4095
            }
            else
            {
                value = 4095 - ((high_phase - 32768) >> 3); // Вниз
            }
...
static void task_print_menu(uint8_t page)
{
    ...
    case 0:
        ...
        {
            print_number_with_fixed_point(TARGET_FREQUENCY_6_25_Hz, 0, 1, 4, 2, 2);
        }
        else if (target_frequency == TARGET_FREQUENCY_10_4_Hz_CODE)
        {
            print_number_with_fixed_point(TARGET_FREQUENCY_10_4_Hz, 0, 1, 4, 1, 2);
        }
        else
        {
...
static bool task_settings(volatile uint8_t *state_settings_menu)
{
    ...
    case ENCODER_CW:
        ...
        {
            if (target_frequency == 24) // target_frequency = 6 Гц
            {
                target_frequency = TARGET_FREQUENCY_6_25_Hz_CODE;
                print_number_with_fixed_point(TARGET_FREQUENCY_6_25_Hz, 0, 1, 4, 2, 2);
                next_phase_inc = pgm_read_dword(&phase_inc_factors[19]);
            ...
            if (target_frequency == TARGET_FREQUENCY_6_25_Hz_CODE) // target_frequency = 6.25 Гц
            {
                target_frequency = 25; // target_frequency = 7 Гц
            ...
            if (target_frequency == 28) // target_frequency = 10 Гц
            {
                target_frequency = TARGET_FREQUENCY_10_4_Hz_CODE;
                print_number_with_fixed_point(TARGET_FREQUENCY_10_4_Hz, 0, 1, 4, 1, 2);
                next_phase_inc = pgm_read_dword(&phase_inc_factors[20]);
                ATOMIC_BLOCK(ATOMIC_RESTORESTATE)
                {
                    phase_inc = next_phase_inc;
                }
            ...
            if (target_frequency == TARGET_FREQUENCY_10_4_Hz_CODE) // target_frequency = 10.4 Гц
            {
                target_frequency = 29; // target_frequency = 11 Гц
                next_phase_inc = (target_frequency - 18) * pgm_read_dword(&phase_inc_factors[18]);
                print_integer_number((target_frequency - 18), 0, 1, 4);
                ATOMIC_BLOCK(ATOMIC_RESTORESTATE)
                {
    ...
    case ENCODER_CCW:
        if (*state_settings_menu == STATE_TASK_MENU_SETTINGS_F)
        {
            if (target_frequency == 29) // target_frequency = 11 Гц
            {
                target_frequency = TARGET_FREQUENCY_10_4_Hz_CODE;
                print_number_with_fixed_point(TARGET_FREQUENCY_10_4_Hz, 0, 1, 4, 1, 2);
                next_phase_inc = pgm_read_dword(&phase_inc_factors[20]);
            ...
            if (target_frequency == TARGET_FREQUENCY_10_4_Hz_CODE) // target_frequency = 10.4 Гц
            {
                target_frequency = 28; // target_frequency = 10 Гц
                next_phase_inc = (target_frequency - 18) * pgm_read_dword(&phase_inc_factors[18]);
                print_integer_number((target_frequency - 18), 0, 1, 4);
                ATOMIC_BLOCK(ATOMIC_RESTORESTATE)
                {
            ...
            if (target_frequency == 25) // target_frequency = 7 Гц
            {
                target_frequency = TARGET_FREQUENCY_6_25_Hz_CODE;
                print_number_with_fixed_point(TARGET_FREQUENCY_6_25_Hz, 0, 1, 4, 2, 2);
                next_phase_inc = pgm_read_dword(&phase_inc_factors[19]);
            ...
            if (target_frequency == TARGET_FREQUENCY_6_25_Hz_CODE) // target_frequency = 6.25 Гц
            {
                target_frequency = 24; // target_frequency = 6 Гц
                next_phase_inc = (target_frequency - 18) * pgm_read_dword(&phase_inc_factors[18]);
                print_integer_number((target_frequency - 18), 0, 1, 4);
                ATOMIC_BLOCK(ATOMIC_RESTORESTATE)
                {
...
static void task_read_settings(void)
{
    target_frequency = (int16_t)eeprom_read_word((uint16_t *)&eep_target_frequency_task_2);
    if ((uint16_t)target_frequency == 0xFFFF)
    {
        eeprom_update_word((uint16_t *)&eep_target_frequency_task_2, (uint16_t)TARGET_FREQUENCY_DEFAULT);
        target_frequency = TARGET_FREQUENCY_DEFAULT;
    }

    target_amplitude = (int16_t)eeprom_read_word((uint16_t *)&eep_target_amplitude_task_2);
    if ((uint16_t)target_amplitude == 0xFFFF)
    {
        eeprom_update_word((uint16_t *)&eep_target_amplitude_task_2, (uint16_t)TARGET_AMPLITUDE_DEFAULT);
        target_amplitude = TARGET_AMPLITUDE_DEFAULT;
    }

    target_mode = (int16_t)eeprom_read_word((uint16_t *)&eep_target_mode_task_2);
    if ((uint16_t)target_mode == 0xFFFF)
    {
        eeprom_update_word((uint16_t *)&eep_target_mode_task_2, (uint16_t)TARGET_MODE_DEFAULT);
...
static void task_save_settings(void)
{
    eeprom_update_word((uint16_t *)&eep_target_frequency_task_2, (uint16_t)target_frequency);
    eeprom_update_word((uint16_t *)&eep_target_amplitude_task_2, (uint16_t)target_amplitude);
    eeprom_update_word((uint16_t *)&eep_target_mode_task_2, (uint16_t)target_mode);
