...
int16_t EEMEM eep_target_frequency_task_1;
int16_t EEMEM eep_target_amplitude_task_1;
int16_t EEMEM eep_target_mode_task_1;
...
// Синтез прямоугольных сигналов для 9-ти каналов ЭКС
void task_1(void)
{
    task_read_settings();
    ...
    while (task_control(&state_menu, &state_signal, true))
    {
        ...
        if (state_signal == STATE_SIGNAL_GENERATE && !is_set_new_dac_values)
        {
            ...
            // MSB определяет полупериод (скважность 2)
...
static void task_read_settings(void)
{
    target_frequency = (int16_t)eeprom_read_word((uint16_t *)&eep_target_frequency_task_1);
    if ((uint16_t)target_frequency == 0xFFFF)
    {
        eeprom_update_word((uint16_t *)&eep_target_frequency_task_1, (uint16_t)TARGET_FREQUENCY_DEFAULT);
        target_frequency = TARGET_FREQUENCY_DEFAULT;
    }

    target_amplitude = (int16_t)eeprom_read_word((uint16_t *)&eep_target_amplitude_task_1);
    if ((uint16_t)target_amplitude == 0xFFFF)
    {
        eeprom_update_word((uint16_t *)&eep_target_amplitude_task_1, (uint16_t)TARGET_AMPLITUDE_DEFAULT);
        target_amplitude = TARGET_AMPLITUDE_DEFAULT;
    }

    target_mode = (int16_t)eeprom_read_word((uint16_t *)&eep_target_mode_task_1);
    if ((uint16_t)target_mode == 0xFFFF)
    {
        eeprom_update_word((uint16_t *)&eep_target_mode_task_1, (uint16_t)TARGET_MODE_DEFAULT);
...
static void task_save_settings(void)
{
    eeprom_update_word((uint16_t *)&eep_target_frequency_task_1, (uint16_t)target_frequency);
    eeprom_update_word((uint16_t *)&eep_target_amplitude_task_1, (uint16_t)target_amplitude);
    eeprom_update_word((uint16_t *)&eep_target_mode_task_1, (uint16_t)target_mode);
